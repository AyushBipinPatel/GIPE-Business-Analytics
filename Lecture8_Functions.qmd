...
## What is happening when we write a function?{.scrollable}

+ What was the logic behind the previous function?
...
+ Each function that you write needs a unique `function_name`, ideally one that easily explains the use of the function
+ `parameters` are the inputs (variables, dataframes etc) that we want our function to take
+ `function_body` is where we instruct the function of the operations we need it to perform
